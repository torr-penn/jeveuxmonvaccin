...
lastcheck timestamp without time zone default now (),
    ...
    total int default 0,
params type character varying(500)
);
...
(
    ...
    centerid int references vaccine_center
(
    id
) on delete cascade,
    lastok timestamp
...
^
torr-penn::DATABASE=> insert into vaccine_center (id,center_id) values (0,0);
INSERT
0 1
...
FROM '/tmp/load_center_sql.csv' WITH DELIMITER ';' ;
...
alter table vaccine_center add column params character varying(200);
...
update vaccine_center set result_topology=b.result_topology, params=b.params, status=2 from center_temp b where vaccine_center.vaccine_id=b.vaccine_id and vaccine_center.center_id=b.center_id;

alter table vaccine_center alter column params type character varying(500);
alter table center_temp alter column 
params type character varying(500);
