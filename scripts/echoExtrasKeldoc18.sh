#!/bin/bash 
echo -n "" > 18k1.txt
echo "### Cotes d armor " >>18k1.txt
cat extra18_22.txt |grep keldoc >>18k1.txt
echo "####FINISTERE Manual 920 crozon - 2393 chateaulin" >> 18k1.txt
cat extra18_29.txt |grep keldoc >> 18k1.txt
echo "####ILLE ET VILAINE - sauf 901 saint MALO" >>18k1.txt
cat extra18_35.txt |grep keldoc >> 18k1.txt
echo "####MORBIHAN" >> 18k1.txt
cat extra18_56.txt |grep keldoc >> 18k1.txt
...
function compare_line(){
	...
	if [ "$cid" != "" ]; then
		echo "update vaccine_center set params18='$paramN' where center_id=$cid;"
		echo " was                              $paramO"
	fi
	fi
	...
	do
		firstchar=`echo $line |cut -c1`
			if [ "$firstchar" != "#" ]; then
				...
				line2=`cat 18k2.txt|grep -o "^$centerid;.*"`
...
function check_from_new(){
	...
	do
		firstchar=`echo $line |cut -c1`
			if [ "$firstchar" != "#" ]; then
				centerid=`echo $line |awk -F\; ' {print $1;}'`
				line2=`cat 18k1.txt|grep -o "^$centerid;.*"`
